...
void (*mqttConnected)(MQTT_Client *client) = NULL;
void (*mqttDisconnected)(MQTT_Client *client) = NULL;
void (*mqttPublished)(MQTT_Client *client) = NULL;
void (*mqttData)(MQTT_Client *client, const char* topic, const char *data, uint32_t len) = NULL;
    ...
    if (len > 5 && strncmp(data, "HAP", 3) == 0)
    {
        ...
        if (strcmp(settings.serverName, hapServer) == 0)
        {
            ...
            if (port != hapPort || address != hapAddress)
            {
                ...
                os_sprintf(aux, "%d", system_get_chip_id());
                MQTT_InitClient(&mqttClient, aux, settings.hapUserName, settings.hapPassword, MQTT_KEEPALIVE, 1);
            ...
            char response[128];
            int length = os_sprintf(response, "%d:%s:%s", system_get_chip_id(), settings.nodeName, node_type);
